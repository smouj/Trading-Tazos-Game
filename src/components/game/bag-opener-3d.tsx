// ============================================================
// Trading Tazos Game — BagOpener3D v8
//
// Premium bag opening. Clean, transparent, interactive.
// - Floating 3D bag on transparent background
// - Real-time cut line visible on the bag surface
// - Refined lighting for natural product look
// - Tazo reveal with particles + screen flash
// ============================================================
"use client"
...
function TazoDisc({ active, color, onRise, onComplete }: {
  ...
  useFrame((_, delta) => {
    ...
    const rawT = Math.max(0, Math.min(1, (elapsed - 0.38) / 0.7))
    ...
    if (ref.current) {
      if (rawT > 0.001) {
        ...
        ref.current.scale.setScalar(0.08 + eased * 1.15)
...
// ══════════════════════════════════════════════════════════
// CUT GUIDE — animated dashed line on the bag seal
// ══════════════════════════════════════════════════════════
function CutGuide({ color, visible }: { color: string; visible: boolean }) {
  const glowRef = useRef(0)
  useFrame((_, delta) => { glowRef.current = (glowRef.current + delta * 1.8) % (Math.PI * 2) })

  if (!visible) return null
  const hw = BAG_W_TOP * 0.44
  const segs = 20
  const ySeal = bodyY + BODY_H / 2 + TOP_CRIMP * 0.45
  const alpha = 0.55 + Math.sin(glowRef.current) * 0.25
  ...
  return (
    <group>
      {/* Main dashed line */}
      <Line points={pts} color={color} lineWidth={2.5} transparent opacity={alpha} dashed dashSize={0.05} gapSize={0.035} depthTest={false} />
      {/* Subtle white outline for contrast */}
      <Line points={pts.map(p => p.clone().add(new THREE.Vector3(0, 0, -0.002)))} color="#ffffff" lineWidth={0.8} transparent opacity={alpha * 0.35} dashed dashSize={0.05} gapSize={0.035} depthTest={false} />
      {/* Endpoint dots */}
      <mesh position={[-hw, ySeal, BAG_D / 2 + 0.028]}>
        <sphereGeometry args={[0.022, 12, 12]} /><meshBasicMaterial color={color} transparent opacity={alpha + 0.15} />
      </mesh>
      <mesh position={[hw, ySeal, BAG_D / 2 + 0.028]}>
        <sphereGeometry args={[0.022, 12, 12]} /><meshBasicMaterial color={color} transparent opacity={alpha + 0.15} />
      </mesh>
...
// ══════════════════════════════════════════════════════════
// TEAR INDICATOR — glowing cut path on the bag surface
// ══════════════════════════════════════════════════════════
function TearIndicator({ points, color }: { points: { x: number; y: number }[]; color: string }) {
  ...
  return (
    <group>
      {/* Primary cut line — thick, franchise color, on surface */}
      <Line points={pts3D} color={color} lineWidth={4} transparent opacity={0.95} depthTest={false} />
      {/* Inner bright core — pure white, thinner */}
      <Line points={pts3D.map(p => p.clone().add(new THREE.Vector3(0, 0, 0.004)))} color="#ffffff" lineWidth={1.8} transparent opacity={0.7} depthTest={false} />
      {/* Glow halo behind the cut */}
      <Line points={pts3D.map(p => p.clone().add(new THREE.Vector3(0, 0, -0.006)))} color={color} lineWidth={6} transparent opacity={0.35} depthTest={false} />
      {/* Spark dots along the path */}
      {pts3D.map((p, i) => i % 4 === 0 ? (
        <mesh key={i} position={p.clone().add(new THREE.Vector3(0, 0, 0.015))}>
...
export default function BagOpener3D({ bag, onOpen, onSkip }: { bag: BagData | null; onOpen: () => void; onSkip: () => void }) {
  ...
  return (
    <div ref={containerRef} className="relative w-full select-none touch-none" style={{ height: canvasH }}>
      ...
      <Canvas
        camera={{ position: [0, 0.02, 2.0], fov: 38 }}
        gl={{ antialias: true, alpha: true, premultipliedAlpha: false }}
        style={{ background: "transparent" }}
        onCreated={({ gl }) => { gl.setClearColor(0x000000, 0); gl.setPixelRatio(Math.min(window.devicePixelRatio, 2)) }}
      >
        {/* Natural product lighting — soft, even, no harsh shadows */}
        <ambientLight intensity={0.85} />
        {/* Key light — soft front-left */}
        <spotLight position={[3.5, 2.5, 5.5]} intensity={3.2} angle={0.4} penumbra={0.5} color="#fffef8" />
        {/* Fill light — soft back-right */}
        <spotLight position={[-2.5, 2, -3.5]} intensity={1.8} angle={0.38} penumbra={0.55} color="#fffef5" />
        {/* Bottom bounce — warms the bag bottom */}
        <pointLight position={[0, -2, 3]} intensity={0.5} color="#ffddbb" />
        {/* Rim light — slight edge definition */}
        <pointLight position={[-1.5, 0, 2.5]} intensity={0.35} color="#ccddff" />

        <Suspense fallback={null}>
          <PotatoChipBag3D
            frontUrl={frontUrl} backUrl={backUrl} scale={1.2}
      ...
      <div className="absolute bottom-4 left-0 right-0 flex items-center justify-center gap-3 z-10 px-4">
        {stage === "idle" && (
          <div className="flex flex-col items-center gap-2">
            <div
              className="px-8 py-3 font-black text-sm uppercase tracking-[0.15em] border-[3px]"
              style={{ backgroundColor: franchiseColor, color: "#1a1a1a", borderColor: "#1a1a1a", boxShadow: "4px 4px 0px #1a1a1a" }}
            >
              ✂ DRAG TO OPEN!
            </div>
            <span className="text-[9px] font-bold text-black/25 uppercase tracking-[0.2em]">slide across the top seal</span>
          </div>
        )}
        {stage === "tearing" && (
          <div className="flex items-center gap-3 w-full max-w-[340px]">
            <div className="flex-1">
              <div className="h-3 bg-black/10 border border-black/10 overflow-hidden rounded">
                <div className="h-full transition-all duration-75 rounded" style={{ width: `${Math.round(tearProgress * 100)}%`, background: franchiseColor, boxShadow: `0 0 12px ${franchiseColor}60` }} />
              </div>
            </div>
