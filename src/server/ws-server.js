...
function makeRoom(p1, p2) {
  ...
  for (const p of [p1, p2]) {
    ...
    broadcast(p.ws, {
      ...
      payload: {
        roomId: id,
        opponent: { userId: opp.userId, name: opp.name },
...
function tryMatch() {
  while (queue.length >= 2) {
    makeRoom(queue.shift(), queue.shift())
  }
}
...
function verifyToken(token) {
  try {
    ...
    return { userId: d.id, name: d.name }
  } catch { return null }
}
    ...
    switch (msg.type) {
      ...
      case "turn_action":
        for (const [, room] of rooms) {
          ...
          if (idx >= 0) {
            ...
            broadcast(room.players[1 - idx].ws, { type: "turn_received", payload: msg.payload })
            break
          }
        }
        break
      case "turn_result":
        for (const [, room] of rooms) {
          const idx = room.players.indexOf(player)
          if (idx >= 0) {
            broadcast(room.players[1 - idx].ws, { type: "turn_result", payload: msg.payload })
            break
          }
        }
        break
